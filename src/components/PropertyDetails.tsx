...
export default function PropertyDetails({ 
    ...
    try {
      ...
      const res = await fetch(`/api/properties/${property.id}/inquiry`, {
        ...
        body: JSON.stringify({
          ...
          senderPhone: quotePhone || currentUser.phone,
          message: formattedMsg
        })
      });

      if (res.ok) {
        setQuoteSuccess('Your Quote Request has been submitted successfully! The supplier will get back to you shortly.');
        setQuoteMessage('');
        setTimeout(() => {
      ...
      setQuoteError('Network error while sending quote request.');
    } finally {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-left details-view animate-fade-in font-sans">
              ...
              <div className="bg-[#0d0d12]/90 rounded-3xl p-6 md:p-8 border border-white/5 shadow-lg text-left text-[#F5F5F4] space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-white/10">
                  <div>
                    <h3 className="text-xl font-serif font-bold text-white flex items-center gap-2">
                      ...
                      <span>{t('reviews.reviews_and_ratings') || 'Reviews & Ratings'}</span>
                    </h3>
                    <p className="text-xs text-white/50 mt-1">
                      {propertyReviews.length} {t('reviews.verified_reviews_count') || 'verified review(s) for this listing'}
                    </p>
                  </div>
                  {avgRating && (
                ...
                <div className="space-y-4 max-h-96 overflow-y-auto pr-1">
                  {propertyReviews.length === 0 ? (
                    <p className="text-xs text-white/40 italic py-3">{t('reviews.no_reviews_yet') || 'No reviews yet.'}</p>
                  ) : (
                ...
                <form onSubmit={handleReviewSubmit} className="pt-4 border-t border-white/10 space-y-3">
                  <h4 className="text-xs font-bold text-white uppercase tracking-wider">{t('reviews.leave_review') || 'Leave a Review'}</h4>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-white/60">{t('reviews.rating') || 'Rating'}:</span>
                  ...
                  >
                    {reviewSubmitting ? (t('reviews.submitting') || 'Submitting...') : (t('reviews.submit_review') || 'Submit Review')}
                  </button>
                </form>
              </div>
            );
      ...
      {sellerListings.filter(p => p.id !== property.id).length > 0 && (
        ...
        {quoteModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-[#0d0d12] border border-amber-500/20 rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl relative text-left text-[#F5F5F4]"
            >
              <button
              ...
              {quoteError && (
                <div className="p-3 bg-red-500/10 border border-red-500/20 text-red-400 text-xs rounded-2xl font-semibold mb-4">
                  {quoteError}
                </div>
              )}

              {quoteSuccess ? (
                <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-xs rounded-2xl font-semibold text-center my-4 animate-pulse">
                  {quoteSuccess}
                </div>
              ) : (
                <form onSubmit={handleSendQuoteRequest} className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full p-3.5 bg-[#12121a] border border-white/10 rounded-2xl text-xs text-white focus:outline-none focus:border-amber-500"
                      placeholder="e.g. Please provide delivery lead time, bulk discount for 500+ units, and payment terms..."
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-black font-extrabold py-3.5 rounded-2xl text-xs uppercase tracking-wider transition shadow-lg cursor-pointer"
                  >
                    {quoteSubmitting ? 'Sending Request...' : 'Submit Official Quote Request'}
                  </button>
                </form>
              )}
            </motion.div>
