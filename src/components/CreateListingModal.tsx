...
export default function CreateListingModal({ onClose }: CreateListingModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-900/60 backdrop-blur-xs flex justify-center items-center p-0 sm:p-4 lg:p-6">
      <div className="bg-[#F8F7F4] w-full h-[100dvh] sm:h-auto sm:max-h-[92vh] sm:max-w-4xl lg:max-w-5xl sm:rounded-3xl overflow-hidden border-0 sm:border sm:border-stone-200 shadow-2xl animate-in fade-in zoom-in-95 duration-200 create-modal-view text-stone-900 flex flex-col">
        
        {/* Header with dynamic title */}
        <div className="p-4 sm:p-5 border-b border-stone-200 flex justify-between items-center bg-white shrink-0">
          <div className="flex items-center gap-2.5">
            <HeaderIcon className="w-5 h-5 text-[#C06853]" />
            <h3 className="font-serif text-base sm:text-lg tracking-wider uppercase font-semibold text-stone-900">
              {getHeaderTitle()}
            </h3>
          </div>
          <button 
            onClick={onClose} 
            className="p-2 rounded-full hover:bg-stone-100 text-stone-400 hover:text-stone-800 transition duration-200 cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {error && (
          <div className="bg-red-50 border-b border-red-200 text-red-600 text-xs py-3 px-5 text-center font-semibold shrink-0">
            ⚠️ {error}
          </div>
        )}
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 create-modal-scroll space-y-6 bg-[#F8F7F4]">
          {currentStep === 1 && (
            <WizardStep1Category
        ...
        {currentStep < 4 && (
          <div className="p-4 sm:p-5 border-t border-stone-200 bg-white flex justify-between items-center gap-3 shrink-0">
            <div>
              {currentStep > 1 ? (
                <button
                  type="button"
                  onClick={() => setCurrentStep((currentStep - 1) as any)}
                  className="px-4 py-2.5 rounded-xl border border-stone-200 hover:bg-stone-50 text-stone-700 text-xs font-bold transition duration-200 cursor-pointer inline-flex items-center gap-1.5"
                >
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="px-4 py-2.5 rounded-xl border border-stone-200 hover:bg-stone-50 text-stone-500 hover:text-stone-800 text-xs font-bold transition duration-200 cursor-pointer"
                >
                  Cancel
                </button>
              )}
            ...
            <div className="flex gap-2">
              {currentStep === 1 && (
                <button
                  ...
                  className="px-6 py-2.5 bg-[#C06853] hover:bg-[#A85340] text-white font-extrabold text-xs rounded-xl shadow-xs transition duration-200 cursor-pointer inline-flex items-center gap-1.5"
                >
                  <span>Continue to Details & Media</span>
